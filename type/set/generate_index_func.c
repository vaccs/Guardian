	...
	struct out_shared* shared)
{
	...
	stringtree_append_printf(text, ""
		"struct type_%u* func_%u(struct type_%u* set, struct type_%u* index)"
		"{"
			"unsigned native_index;"
			""
			"if (mpz_fits_uint_p(index->value) && (native_index = mpz_get_ui(index->value)) < set->n)"
			"{"
				"return func_%u(set->data[native_index]);"
			"}"
			"else"
			"{"
