	...
	struct type* environment)
{
	...
	stringtree_append_printf(tree, "unsigned len = left->len + right->len;");
	
	stringtree_append_printf(tree, "unsigned char* buffer = malloc(len);");
	
	stringtree_append_printf(tree, "memcpy(mempcpy(buffer, left->chars, left->len), right->chars, right->len);");
	
	stringtree_append_printf(tree, "struct type_%u* concat = func_%u(buffer, len);", super->type->id, new_id);
