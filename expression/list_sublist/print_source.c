...
struct stringtree* list_sublist_expression_print_source(
	...
	struct environment_type* environment)
{
	...
	assert(super->kind == ek_list_sublist);
	...
	if (this->startindex)
	{
		...
		stringtree_append_printf(tree, ""
			...
			"{"
				"fprintf(stderr, \"%%s: cannot generate sublist starting from index %%u in list %%u elements long!\\n\", argv0, native_start, list->n);"
				"exit(1);"
			"}"
		"");
	...
	if (this->endindex)
	{
		...
		stringtree_append_printf(tree, ""
			...
			"{"
				"fprintf(stderr, \"%%s: cannot generate sublist ending on index %%u in list %%u elements long!\\n\", argv0, native_end, list->n);"
				"exit(1);"
			"}"
		"");
