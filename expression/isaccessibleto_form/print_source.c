...
struct stringtree* isaccessibleto_form_expression_print_source(
	...
	struct environment_type* environment)
{
	...
	
	{
		...
		stringtree_append_printf(tree, "int sysret = faccessat(AT_FDCWD, (char*) native_path, R_OK, AT_EACCESS);");
		
		stringtree_append_printf(tree, "printf(\"%%s: access(\\\"%%s\\\", R_OK) = %%i\\n\", argv0, native_path, sysret);");
		
		stringtree_append_printf(tree, ""
			"struct type_%u* result = func_%u(sysret == 0);",
